...
function EvaluacionCursos() {
    ...
    var intProm = 10 / 12;
    d.getElementById("CE_CALIF").value = intCalificacion * intProm;
}
...
function LoadGridEjecutivo() {
    ...
    var strMeta = "";
    var strFac = "";
    var strTotalVendido = 0;
    var strTotalFacturado = 0;
    var strTotalPagado = 0;
    var strMetaTotal = 0;
    ...
    var strStyle = "<style>"
            ...
            + "a{color: #FF6500; fonr: bold;}"
            + ".fondoTabla{background-color: #E6E6E6;}"
            + ".table1{width: 90%;border: black 3px solid;margin: auto; border-collapse: collapse;  background-color: #E6E6E6;}"
            + ".tr1{border: black 3px solid;border-collapse: collapse; }"
            + ".td1{border: black 3px solid;border-collapse: collapse; width: 70px;}"
            + ".ejecutivo{font-size: 25px;}"
            + ".CenterText{text-align: center;}"
            + ".able{color: #00D827;}"
            + ".disable{color: gray;}"
            + ".oks{font-size: 11px;color: #FF6500;}"
...
//    $("#dialogWait").dialog("open");
    $.ajax({
        ...
        success: function (datos) {
            ...
            for (var i = 0; i < lstCte.length; i++) {
                ...
                strRegistro = objcte.getAttribute("REGISTROS");
                ...
                strTotalVendido += parseFloat(strReserva);
                strTotalFacturado += parseFloat(strFac);
                strTotalPagado += parseFloat(strCobro);
                strMetaTotal += parseFloat(strMeta);

                strContenido += "<tr class='tr1'>"
                        + "<td class=' td1 ejecutivo CenterText'>"
                        + "<div class='ejecutivo' title='SELECCIONAR EJECUTIVO'><a href='javascript:DatosMonitoreo(" + strExt + "," + intEvalua + ",\"" + strIP + "\",\"" + strEjecutivo + "\"," + strId_Ejecutivo + ")'>" + strEjecutivo + "</a></div>"
                        + "</td>"
                        + "<td class='td1'>"
                        + "<div>" + strBase + "</div>"
                        + "<div>Total Reg.: " + strRegistro + "</div>"
                        + "<div>"
                        + "<i class='fa fa-user able' title='NUMERO DE EX PARTICIPANTES'></i> " + strExp
                        + "&nbsp;&nbsp;"
                        + "<i class='fa fa-user disable' title='NUMERO DE PROSPECTOS'></i> " + strPros
                        + "</div>"
                        + "</td>"
                        + "<td class='td1' style='width: 80px'>"
                        + "<div><i class='fa fa-phone' title='NUMERO DE LLAMADAS'></i> " + intNLlamada + "</div>"
                        + "<div ><i class='fa fa-commenting-o' title='NUMERO DE NOTAS'></i> " + strNota + "</div>"
                        + "</td>"
                        + "<td class='td1'><div class='wait'>importe vendido</div>"
                        + "<br>"
                        + "<i class='fa fa-shopping-cart able' title='VENDIDO'></i> " + FormatNumber(strReserva, 2, true, false, true) + ""
                        + "</td>"
                        + "<td class='td1'><div class='oks'>importe facturado</div>"
                        + "<br>"
                        + "<i class='fa fa-file-text-o disable' title='FACTURADO'></i> " + FormatNumber(strFac, 2, true, false, true) + ""
                        + "</td>"
                        + "<td class='td1'><div class='oks'>importe cobrado </div>"
                        + "<br>"
                        + "<i class='fa fa-usd able' title='COBRADO'></i> " + FormatNumber(strCobro, 2, true, false, true) + ""
                        + "</td>"
                        + "<td class='td1'><div class='wait'>importe meta</div>"
                        + "<br>"
                        + "<i class='fa fa-star-o disable' title='META'></i> " + FormatNumber(strMeta, 2, true, false, true) + ""
                        + "</td>"
                        + "</tr>";
            }
            var strPieTabla = "<tr>"
                    ...
                    + "<td class='td1'><table><tr><td>"
                    + "<b>TOTAL VENDIDO</b>"
                    + "</td></tr><tr><td>"
                    + "<i class='fa fa-shopping-cart able' title='TOTAL VENDIDO'></i> " + FormatNumber(strTotalVendido, 2, true, false, true)
                    + "</td></tr></table></td>"

                    + "<td class='td1'><table><tr><td>"
                    + "<b>TOTAL FACTURADO</b>"
                    + "</td></tr><tr><td>"
                    + "<i class='fa fa-file-text-o disable' title='TOTAL FACTURADO'></i> " + FormatNumber(strTotalFacturado, 2, true, false, true)
                    + "</td></tr></table></td>"

                    + "<td class='td1'><table><tr><td>"
                    + "<b>TOTAL PAGADO</b>"
                    + "</td></tr><tr><td>"
                    + "<i class='fa fa-usd able' title='TOTAL COBRADO'></i> " + FormatNumber(strTotalPagado, 2, true, false, true)
                    + "</td></tr></table></td>"

                    + "<td class='td1'><table><tr><td>"
                    + "<b>META TOTAL</b>"
                    + "</td></tr><tr><td>"
                    + "<i class='fa fa-star-o disable' title='META TOTAL'></i> " + FormatNumber(strMetaTotal, 2, true, false, true)
                    + "</td></tr></table></td>"

                    + "</tr>";
            var strTablaFin = "</table>"
            "</div>";
            strHtmlTabla = strStyle + strTabla + strContenido + strPieTabla + strTablaFin;
    ...
    timeTimer = setTimeout('LoadGridEjecutivo()', 120000); //2 minutos
}
...
function saveEval(numPaso) {
    ...
    var intCalif = 0;
    intCalif = document.getElementById("CE_CALIF").value;
...
function HtmlBtn() {
    ...
    var strHtmlBtnEv = "";
    if (strExt != "") {
        strHtmlBtnEv = "<table border='0' width='0%' style='padding-left: 60%'>"
                + "<tr>"
                ...
                + '<td align="center"><a href="sip:*9' + strExt + '" class=\'cofide_evaluacion\'><i class = "fa fa-volume-up" title="Llamada" style="font-size:60px; width:110px"></i></td>'
//            + '<td align="center"><div class=\'cofide_evaluacion\'><i class = "fa fa-volume-up" title="Llamada" style="font-size:60px; width:110px"></i></td>'
        ...
        strHtmlBtnEv = "<table border='0' width='0%' style='padding-left: 60%'>"
                ...
                + '<td align="center"><a href="javascript:ViewUsr()" class=\'cofide_evaluacion\'><i class = "fa fa-video-camera" title="Monitoreo" style="font-size:60px; width:110px"></i></td>'
                + '<td align="center"><a href="javascript:SelEvaluacion(1)" class=\'cofide_evaluacion\'><i class = "fa fa-check-circle" title="Evaluación" style="font-size:60px; width:110px"></i></td>'
                + '<td align="center"><a href="javascript:ExtAgente(' + strExt + ')" class=\'cofide_evaluacion\'><i class = "fa fa-volume-up" title="Llamada" style="font-size:60px; width:110px"></i></td>'
//            + '<td align="center"><div class=\'cofide_evaluacion\'><i class = "fa fa-volume-up" title="Llamada" style="font-size:60px; width:110px"></i></td>'
                + "</tr>";
        "</table>";
    }
...
function ExtAgente(strExt) {
    if (strExt == "") {
        alert("No se Encuentra la Extensión");
    }
}
function CuentaLlamada() {
    var intId_Usr = document.getElementById("CE_ID_USER").value;
