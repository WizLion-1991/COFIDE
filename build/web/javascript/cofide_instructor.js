...
function subirFoto() {
    var strFoto = document.getElementById("CI_FOTO_");
    var File = document.getElementById("CI_FOTO_").value;
    if (strFoto != "") {
        if (Right(strFoto.value.toUpperCase(), 3) == "JPG" || Right(strFoto.value.toUpperCase(), 3) == "PNG") {
            $.ajaxFileUpload({
                url: "COFIDE_UpMaterial.jsp?ID=3",
                secureuri: false,
                fileElementId: "CI_FOTO_",
                dataType: "json",
                success: function (data, status) {
                    if (typeof (data.error) != "undefined") {
                        if (data.error != "") {
                            ...
                            var strFile = File.replace("C:fakepath", "");
                            document.getElementById("CI_FOTO").value = strFile;
                        }
                    }
...
function UpdateInstructor() {
    ...
    var strFoto = document.getElementById("CI_FOTO").value;
    strFoto = strFoto.replace("C:fakepath", "");
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "html",
        url: "COFIDE_Pago_Instructores.jsp?ID=4",
...
function CargaEvaluacion() {
    var intId = document.getElementById("CI_INSTRUCTOR_ID").value;
    var strFecini = document.getElementById("INS_FEC_INI").value;
    var strFecFin = document.getElementById("INS_FEC_FIN").value;
    var strPost = "";
    strPost = "fec_ini=" + strFecini;
    strPost += "&fec_fin=" + strFecFin;
    strPost += "&Instructor=" + intId;
    strPost += "&opc=2"; //viene desde instructores
    ...
    $.ajax({
        ...
        success: function (datos) {
            jQuery("#GRID_INSTRUCTOR").clearGridData();
            ...
            for (var i = 0; i < lstCI.length; i++) {
                ...
                jQuery("#GRID_INSTRUCTOR").addRowData(getMaxGridCursosImpartir("#GR_INSTRUCTOR"), rowCursos, "last");
            }
            $("#dialogWait").dialog("close");
        },
        error: function (objeto, quepaso, otroobj) {
