...
<%
    ...
    if (varSesiones.getIntNoUser() != 0 && seg.ValidaURL(request)) {
        ...
        if (strid != null) {
            ...
            //revisamos el estado de quartz, para activarlo, si aun no se encuentra activo
            if (strid.equals("5")) {
                String strRespuesta = "ON";
                boolean bolExiste = false;
                String strSql = "select * from cofide_quartz_estatus where Q_FECHA = '" + fec.getFechaActual() + "';";
                ResultSet rs = oConn.runQuery(strSql, true);
                while (rs.next()) {
                    bolExiste = true;
                }
                ...
                if (!bolExiste) {
                    strRespuesta = "OFF";
                    strSql = "INSERT INTO cofide_quartz_estatus (Q_FECHA, Q_USR) VALUES ('" + fec.getFechaActual() + "', '" + varSesiones.getIntNoUser() + "')";
                    oConn.runQueryLMD(strSql);
                    if (oConn.isBolEsError()) {
                        strRespuesta = "Hubó un problema al Registrar la actividad del motor. " + oConn.getStrMsgError();
