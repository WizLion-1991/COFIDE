...
//        miCuenta.ObtenDatos(1, oConn);
        ...
        for (int i = 0; i < lstFac.length; i++) {
            ...
            String strSql = "select FAC_ID,CT_ID,FAC_FOLIO,FAC_TIPOCOMP,FAC_NOMFORMATO,EMP_ID,FAC_RAZONSOCIAL,FAC_FECHA,FAC_ES_CFD,FAC_ES_CBB,FAC_US_ALTA from vta_facturas where FAC_ID = " + intFAC_ID;
            if (bolEsNC) {
            ...
            while (rs.next()) {
                if (!bolEsNC) {
                    ...
                    intusAlta = rs.getInt("FAC_US_ALTA");
                } else {
            ...
            String strMailCte = getStrCorreoFactura(oConn, intFAC_ID);
            if (intusAlta == 0) {
                intusAlta = varSesiones.getIntNoUser();
            }
            strSql = "select SMTP_US from usuarios where id_usuarios = " + intusAlta;
            rs = oConn.runQuery(strSql, true);
            //Buscamos el nombre del archivo
            while (rs.next()) {
                strVIEW_COPIA = rs.getString("SMTP_US");
            }
            rs.close();
            ...
            if (!strMailCte.equals("") || !strVIEW_COPIA.equals("")) {
                ...
                //Mandamos el mail
                String strMiCuenta[] = micuenta(oConn, intusAlta);
                Mail mail = new Mail();
                mail.setBolDepuracion(false);
                if (strMiCuenta[4].equals("1")) {
                    mail.setBolUsaTls(true);
                    System.out.println("tls: " + strMiCuenta[4]);
                }
                if (strMiCuenta[5].equals("1")) {
                    mail.setBolUsaStartTls(true);
                    System.out.println("stls: " + strMiCuenta[5]);
                }
                mail.setHost(strMiCuenta[2]);
                System.out.println("host: " + strMiCuenta[2]);
                mail.setUsuario(strMiCuenta[0]);
                System.out.println("usuario: " + strMiCuenta[0]);
                mail.setContrasenia(strMiCuenta[1]);
                System.out.println("contraseña:  " + strMiCuenta[1]);
                mail.setPuerto(strMiCuenta[3]);
                System.out.println("puerto:  " + strMiCuenta[3]);
                //Adjuntamos archivos
                ...
                if (!bol) {
                    ...
                    strResp = "FACTURA ENVIADA CON EXITO.";
                }
            } else {
...
<%!   /**
    ...
    public String getStrCorreoFactura(Conexion oConn, int intFAC_ID) {
        String strSql = "";
        String strCorreo = "";
        int intDFA_ID = 0;
        strSql = "select DFA_ID from vta_facturas where FAC_ID = " + intFAC_ID;
        try {
            ResultSet rs = oConn.runQuery(strSql, true);
            while (rs.next()) {
            ...
            System.out.println("GET DFA_ID: [" + ex.getLocalizedMessage() + "]");
        }
        if (intDFA_ID != 0) {
            strSql = "select DFA_EMAIL from vta_cliente_facturacion where DFA_ID = " + intDFA_ID;
            try {
                ResultSet rs = oConn.runQuery(strSql, true);
                while (rs.next()) {
                    if (rs.getString("DFA_EMAIL") != null) {
                        strCorreo = rs.getString("DFA_EMAIL");
                        System.out.println("################## correo al que se le envia la factura: " + strCorreo);
                    }
                }
                rs.close();
            } catch (SQLException ex) {
                System.out.println("GET DFA_EMAIL [" + ex.getLocalizedMessage() + "]");
            }
        }
        return strCorreo;
    }

    public String[] micuenta(Conexion oConn, int strUsuario) {
        String[] datos = new String[6];
        String strSql = "select SMTP, SMTP_US, SMTP_PASS, PORT, SMTP_USASTLS, SMTP_USATLS from usuarios where id_usuarios = " + strUsuario;
        try {
            ResultSet rs = oConn.runQuery(strSql, true);
            while (rs.next()) {
                datos[0] = rs.getString("SMTP_US");
                datos[1] = rs.getString("SMTP_PASS");
                datos[2] = rs.getString("SMTP");
                datos[3] = rs.getString("PORT");
                datos[4] = rs.getString("SMTP_USATLS");
                datos[5] = rs.getString("SMTP_USASTLS");
            }
            rs.close();
        } catch (SQLException sql) {
            System.out.println("Error al obtener la información del usuario: " + sql.getMessage());
        }
        return datos;
    }
%>