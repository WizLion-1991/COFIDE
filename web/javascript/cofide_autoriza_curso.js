 ...
 */
var strCnfCursoEdita = 0;
var strCnfCursoClave = 0;
var strCnfCursoDescripcion = 0;
var strCnfMesConsulta = 0;
var strCnfAnioConsulta = 0;
...
function OpnProgramCursoEdit() {
    if (document.getElementById("CI_MES").value > 0 || document.getElementById("CI_MES").value != "0") {
        ...
        if (grid.getGridParam("selrow")) {
            ...
            strCnfCursoDescripcion = lstVal.CC_NOMBRE_CURSO;
            strCnfCursoClave = lstVal.CC_CLAVES;
            strCnfCursoEdita = lstVal.CC_CURSO_ID;
            strCnfMesConsulta = document.getElementById("CI_MES").value;
            strCnfAnioConsulta = document.getElementById("CI_ANIO").value;
            myLayout.open("west");
            myLayout.open("east");
            myLayout.open("south");
            myLayout.open("north");
...
function publicaCurso() {
    ...
    if (grid.getGridParam("selrow")) {
        ...
        strPost = "CC_CURSO_ID=" + lstVal.CC_CURSO_ID;
        strPost += "&publica=1";
        if (cmfInst == "SI" && FichaTec == "SI") {
            $("#dialogWait").dialog("open");
            $.ajax({
                type: "POST",
                data: strPost,
...
function CancelInstructor() {
    ...
    if (grid.getGridParam("selrow")) {
        ...
        $.ajax({
            ...
            cache: false,
            dataType: "html",
            url: "COFIDE_AutorizaCurso.jsp?id=3",
            success: function (datos) {
                if (datos.substring(0, 2) == "OK") {
                    getCursosConfirm();
                    $("#dialogWait").dialog("close");
        ...
        alert("Selecciona un curso de la lista.");
    }
}