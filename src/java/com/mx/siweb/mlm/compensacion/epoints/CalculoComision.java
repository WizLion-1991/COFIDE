 ...
 */
package com.mx.siweb.mlm.compensacion.epoints;
...
public class CalculoComision extends Comision_MLM {
   ...
   public void doFase1() {
      ...
      log.info("Inicia Fase 1.... " + fecha.getFechaActualDDMMAAAADiagonal() + " " + fecha.getHoraActualHHMMSS());
...
//               if(rs2.getStatement() != null )rs2.getStatement().close(); 
         ...
         while (rs.next()) {
            ...
            if (mapaClientes.containsKey(rs.getInt("CT_ID"))) {
               MlmClienteEpoints cteMlm = (MlmClienteEpoints) mapaClientes.get(rs.getInt("CT_ID"));
               ...
               MlmClienteEpoints cteMlm = new MlmClienteEpoints();
         ...
         while (it.hasNext()) {
            ...
            intContaProcess++;
            intContaProcessTot++;
            MlmClienteEpoints cteMlm = (MlmClienteEpoints) e.getValue();
            //Guardamos info
   ...
   public void doFase3() {
      ...
      ArrayList<MlmClienteEpoints> lstMlmCliente = new ArrayList<MlmClienteEpoints>();
      ...
      try {
         ...
         while (rs.next()) {
            intConta++;
            MlmClienteEpoints cte = new MlmClienteEpoints();
      ...
      for (int i = lstMlmCliente.size() - 1; i >= 0; i--) {
         MlmClienteEpoints cte = lstMlmCliente.get(i);
...
//         if (intIdx != -1) {
//            MlmClienteEpoints ctePadre = lstMlmCliente.get(intIdx);
...
//         }
         ...
         if (intIdx != -1) {
            ...
            if (intIdx2 != -1) {
               MlmClienteEpoints ctePadre2 = lstMlmCliente.get(intIdx2);
               ...
               //Si se encontro el abuelo
               if (intIdx3 != -1) {
                  MlmClienteEpoints ctePadre3 = lstMlmCliente.get(intIdx3);
                  ...
                  //Si se encontro el abuelo
                  if (intIdx4 != -1) {
                     MlmClienteEpoints ctePadre4 = lstMlmCliente.get(intIdx4);
...
//         }
   ...
   private void doDetectaKit() {
      ...
      String strSql = "select vta_tickets.CT_ID, vta_ticketsdeta.TKTD_CVE from  vta_tickets, vta_ticketsdeta  \n"
         ...
         + "  and vta_ticketsdeta.TKTD_CVE in ('MEMBRESIA2000' ,'MEMBRESIA800') "
            ...
            while (rs2.next()) {
               if (rs2.getInt("CT_UPLINE") == 4 && rs2.getInt("CT_ACTIVO") == 0) {
                  ...
                  if (intActivoPadre == 1) {
                     ...
                     ActivaBinario activador = new ActivaBinario(this.oConn);
                     activador.setIntUplineInicial(2);
                     activador.setIntUplineTemporal(4);
                     activador.activarDistribuidor(intCte);
                  }

                  if (rs.getString("TKTD_CVE").equals("MEMBRESIA2000")) {
                     //Llamamos objeto para guardar los datos de la tabla
                     MlmMovComis objTabla = new MlmMovComis();

                     objTabla.setFieldString("MMC_FECHA", fecha.getFechaActual());
                     objTabla.setFieldInt("CT_ID", rs2.getInt("CT_ID"));
                     objTabla.setFieldDouble("MMC_CARGO", 0);
                     objTabla.setFieldDouble("MMC_ABONO", 250);
                     objTabla.setFieldString("MMC_NOTAS", "Kit");
                     objTabla.setFieldInt("MMC_USUARIO", 1);
                     objTabla.setFieldInt("FAC_ID", 0);
                     objTabla.setFieldInt("MC_ID", 0);
                     objTabla.setFieldInt("NC_ID", 0);
                     objTabla.setFieldInt("MPE_ID", this.intPeriodo);

                     objTabla.Agrega(this.oConn);
