...
//               if(rs2.getStatement() != null )rs2.getStatement().close(); 
         ...
         while (rs.next()) {
            ...
            if (mapaClientes.containsKey(rs.getInt("CT_ID"))) {
               MlmClienteMultimatrix cteMlm = (MlmClienteMultimatrix) mapaClientes.get(rs.getInt("CT_ID"));
               ...
               MlmClienteMultimatrix cteMlm = new MlmClienteMultimatrix();
         ...
         while (it.hasNext()) {
            ...
            intContaProcess++;
            intContaProcessTot++;
            MlmClienteMultimatrix cteMlm = (MlmClienteMultimatrix) e.getValue();
            //Guardamos info
   ...
   public void doFase3() {
      ...
      ArrayList<MlmClienteMultimatrix> lstMlmCliente = new ArrayList<MlmClienteMultimatrix>();
      ...
      try {
         ...
         while (rs.next()) {
            intConta++;
            MlmClienteMultimatrix cte = new MlmClienteMultimatrix();
      ...
      for (int i = lstMlmCliente.size() - 1; i >= 0; i--) {
         MlmClienteMultimatrix cte = lstMlmCliente.get(i);
...
//         if (intIdx != -1) {
//            MlmClienteMultimatrix ctePadre = lstMlmCliente.get(intIdx);
...
//         }
         ...
         if (intIdx != -1) {
            ...
            if (intIdx2 != -1) {
               MlmClienteMultimatrix ctePadre2 = lstMlmCliente.get(intIdx2);
               ...
               //Si se encontro el abuelo
               if (intIdx3 != -1) {
                  MlmClienteMultimatrix ctePadre3 = lstMlmCliente.get(intIdx3);
                  ...
                  //Si se encontro el abuelo
                  if (intIdx4 != -1) {
                     MlmClienteMultimatrix ctePadre4 = lstMlmCliente.get(intIdx4);
